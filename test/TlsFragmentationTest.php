...
class TlsFragmentationTest extends AsyncTestCase
{
    public function testTls(): void
    {
        ...
        delay(0.1); // Delay until after the server has closed the connection.
    ...
    private function read(ByteStream\ReadableStream $source, int $minLength): string
    {
        ...
        throw new \RuntimeException("Stream ended prior to {$minLength} bytes being read; received: {$buffer}");
    }
}
